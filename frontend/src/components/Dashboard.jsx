...
export default function Dashboard({ setToken }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const [userResponse, iotResponse, historyResponse] = await Promise.all([
        ...
        axios.get('http://localhost:5000/iot-data/history', config)
      ]);
  ...
  const chartOptions = {
    ...
    scales: {
      x: { 
        grid: { display: false },
        title: { display: true, text: 'Hora', color: '#616161' }
      },
      y: { 
        min: 0, 
        max: 100,
                    ...
                    <CardContent>
                      <Box sx={{ 
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {iotData?.timestamp ? new Date(iotData.timestamp).toLocaleTimeString() : 'N/A'}
                        </Typography>
                      </Box>
